...
public class MindMapRepository {
    ...
    public void saveNode(String mapId, Node node) {
        String sql = "INSERT OR REPLACE INTO nodes " +
                     "(id, mind_map_id, text, parent_id, x_coordinate, y_coordinate, text_size, color, shape) " +
                     "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        try (Connection conn = DatabaseManager.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    public void updateNode(Node node) {
        String sql = "UPDATE nodes SET text = ?, x_coordinate = ?, y_coordinate = ?, text_size = ?, color = ?, shape = ? WHERE id = ?";
        try (Connection conn = DatabaseManager.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    public List<MindMap> loadAll(String userId) {
        ...
        String nodeSql = "SELECT id, text, parent_id, x_coordinate, y_coordinate, text_size, color, shape " +
